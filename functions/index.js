...
const { createQuoteCheckoutSession, handleStripeWebhook } = require("./stripe");
const { submitDemande } = require("./demandes");

const createPaymentHttp = onRequest({ region: "europe-west9", cors: true }, async (req, res) => {
  ...
  try { return res.status(200).json(await createPayment(req.body)); }
    ...
    if (error instanceof CreatePaymentError) return res.status(error.code === "INTERNAL_ERROR" ? 500 : 400).json({ ok: false, code: error.code, message: error.message });
    console.error("Erreur createPayment non prévue :", error);
    return res.status(500).json({ ok: false, code: "INTERNAL_ERROR", message: "Une erreur interne est survenue." });
  }
});

const createQuotePaymentHttp = onRequest({ region: "europe-west9", cors: true }, async (req, res) => {
  if (req.method !== "POST") { res.set("Allow", "POST"); return res.status(405).json({ ok: false, code: "METHOD_NOT_ALLOWED", message: "Method Not Allowed" }); }
...
const stripeWebhook = onRequest({ region: "europe-west9", cors: false }, async (req, res) => {
  if (req.method !== "POST") { res.set("Allow", "POST"); return res.status(405).send("Method Not Allowed"); }
  try {
    const signature = req.get("stripe-signature");
    if (!req.rawBody) return res.status(400).json({ ok: false, code: "RAW_BODY_REQUIRED", message: "Corps brut Stripe indisponible." });
    return res.status(200).json(await handleStripeWebhook(req.rawBody, signature));
...
function projectCatalogueItem(item, { product = false } = {}) {
  const projected = { id: item?.id, nom: item?.nom, prix: item?.prix, ordre: item?.ordre, actif: item?.actif, stockDisponible: item?.stockDisponible, description: item?.description, photo: item?.photo, composition: item?.composition };
  if (product) projected.categorie = item?.categorie;
  return projected;
}
...
module.exports = { getFormules, getProduits, getCommandesConfig, createPayment: createPaymentHttp, createQuotePayment: createQuotePaymentHttp, stripeWebhook, getCatalogue, submitDemande };
