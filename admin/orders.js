...
function statusClass(status) {
  ...
  return { nouvelle: "new", en_preparation: "preparing", prete: "ready", terminee: "completed", annulee: "cancelled" }[safeStatus] || "unknown";
}

function isPaidOrder(order) {
  return String(order?.paiement?.statut || "").trim().toLowerCase() === "paye" && String(order?.paiement?.provider || "").trim().toLowerCase() === "stripe";
}
...
async function loadOrders() {
  ...
  try {
    ...
    if (selectedOrder) {
      ...
      if (selectedOrder && isPaidOrder(selectedOrder)) renderDetail(selectedOrder);
      else closeDetail();
    }
    setListStatus("", false);
  } catch (error) {
...
function renderList() {
  ...
  if (!filtered.length) {
    elements.list.innerHTML = `<div class="admin-empty-state compact"><span class="admin-empty-icon">▣</span><h3>Aucune commande</h3><p>Aucune commande payée ne correspond au filtre sélectionné.</p></div>`;
    return;
  }
  ...
  filtered.forEach((order) => {
    ...
    const status = normalizeStatus(order.statut);
    const amount = formatAmount(order.montantTotal ?? (Number(order.montants?.totalCentimes || 0) / 100));
    card.innerHTML = `<div class="admin-order-main"><div class="admin-order-title-line"><strong>${escapeHtml(order.numeroCommande || order.id)}</strong><span class="admin-order-status admin-order-status-${escapeHtml(statusClass(status))}">${escapeHtml(STATUS_LABELS[status] || displayValue(order.statut))}</span></div><div class="admin-order-meta"><span>${escapeHtml(formatDate(order.createdAt || order.horodateur))}</span><span>${escapeHtml(clientName)}</span><span>${escapeHtml(displayValue(order.modeReception))}</span><span>${escapeHtml(displayValue(order.creneau))}</span><strong>${escapeHtml(amount)}</strong></div>${order.client?.telephone ? `<div class="admin-order-phone">${escapeHtml(order.client.telephone)}</div>` : ""}</div><button type="button" class="btn btn-secondary admin-order-view" data-order-id="${escapeHtml(order.id)}">Voir</button>`;
...
async function saveSelectedStatus() {
  ...
  if (!STATUS_VALUES.includes(nextStatus)) { showStatusMessage("Statut invalide.", true); return; }
  ...
  try {
    ...
    renderList(); renderDetail(selectedOrder); showStatusMessage("Statut enregistré.", false);
  } catch (error) { console.error("Erreur de mise à jour du statut de commande :", error); showStatusMessage(`Impossible d’enregistrer le statut : ${error?.message || "erreur inconnue"}`, true); }
  finally { elements.detailSave.disabled = false; }
}

function showStatusMessage(message, isError) { if (!elements.detailStatusMessage) return; elements.detailStatusMessage.textContent = message; elements.detailStatusMessage.className = isError ? "admin-alert admin-alert-error" : "muted"; }
function setListStatus(message, isError) { if (!elements.listStatus) return; elements.listStatus.textContent = message; elements.listStatus.className = isError ? "admin-alert admin-alert-error" : "admin-alert"; elements.listStatus.hidden = !message; }
function normalizeStatus(value) { const text = normalizeText(value); const aliases = { nouvelle: "nouvelle", new: "nouvelle", "en preparation": "en_preparation", en_preparation: "en_preparation", preparing: "en_preparation", prete: "prete", ready: "prete", terminee: "terminee", completed: "terminee", annulee: "annulee", cancelled: "annulee" }; return aliases[text] || String(value || "").trim(); }
...
function formatAmount(value) { if (typeof value === "number" && Number.isFinite(value)) return new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR" }).format(value); if (typeof value === "string" && value.trim() !== "" && Number.isFinite(Number(value.replace(",", ".")))) return new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR" }).format(Number(value.replace(",", "."))); return value === undefined || value === null || String(value).trim() === "" ? "Non renseigné" : String(value); }
