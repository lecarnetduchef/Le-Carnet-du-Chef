...
function mount() {
  ...
  section.innerHTML = `
    <div class="admin-section-heading">
      <div><p class="admin-eyebrow">COMMERCIAL</p><h2>Devis</h2></div>
      <button type="button" class="btn btn-secondary" data-quote-refresh>↻ Actualiser</button>
    </div>
    <div class="lcc-quote-toolbar">
      <div class="lcc-quote-stats"><span><b data-q-count>0</b> devis</span><span><b data-q-total>0 €</b> montant proposé</span><span><b data-q-open>0</b> en attente</span></div>
      <button type="button" class="btn btn-primary" data-quote-new>+ Nouveau devis</button>
    </div>
    <div class="lcc-quote-layout">
      <div class="lcc-quote-list" data-q-list></div>
      <aside class="lcc-quote-editor" data-q-editor><div class="lcc-demande-empty">Sélectionnez un devis ou créez-en un nouveau.</div></aside>
    </div>`;
...
function openEditor(quote = null) {
  selectedId = quote?.id || null;
  renderEditor(selectedId, quote);
}
...
function renderEditor(id, supplied = null) {
  ...
  editor.innerHTML = `
    <div class="lcc-quote-editor-head"><div><p class="admin-eyebrow">${q.id ? "DEVIS" : "NOUVEAU DEVIS"}</p><h3>${esc(q.numero || "Préparation du devis")}</h3></div>${q.id ? `<button type="button" class="btn btn-secondary" data-q-delete>Supprimer</button>` : ""}</div>
    <form data-q-form>
      <div class="admin-form-grid">
        <div class="form-field"><label>Numéro</label><input name="numero" value="${esc(q.numero || `DEV-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`)}" required></div>
        <div class="form-field"><label>Statut</label><select name="statut">${STATUS.map(([id,text]) => `<option value="${id}" ${id === (q.statut || "brouillon") ? "selected" : ""}>${text}</option>`).join("")}</select></div>
        <div class="form-field"><label>Client</label><input name="clientNom" value="${esc(q.clientNom || "")}" required></div>
        <div class="form-field"><label>Email</label><input name="clientEmail" type="email" value="${esc(q.clientEmail || "")}"></div>
        <div class="form-field"><label>Téléphone</label><input name="clientTelephone" value="${esc(q.clientTelephone || "")}"></div>
        <div class="form-field"><label>Date du devis</label><input name="dateDevis" type="date" value="${esc(q.dateDevis || new Date().toISOString().slice(0,10))}"></div>
        <div class="form-field admin-field-full"><label>Objet / prestation</label><input name="objet" value="${esc(q.objet || "")}" placeholder="Ex. prestation traiteur, chef à domicile…"></div>
      </div>
      <div class="lcc-lines"><div class="lcc-lines-head"><h4>Prestations</h4><button type="button" class="btn btn-secondary" data-q-add-line>+ Ajouter une ligne</button></div><div data-q-lines>${lines.map((line, i) => lineHtml(line, i)).join("")}</div></div>
      <div class="lcc-quote-total"><span>Total</span><strong data-q-total-editor>${money(q.total)}</strong></div>
      <div class="form-field"><label>Conditions / notes</label><textarea name="notes" rows="4">${esc(q.notes || "")}</textarea></div>
      <div class="admin-form-actions"><button type="submit" class="btn btn-primary">${q.id ? "Enregistrer le devis" : "Créer le devis"}</button>${q.clientEmail ? `<a class="btn btn-secondary" href="mailto:${esc(q.clientEmail)}?subject=${encodeURIComponent(q.numero || "Votre devis — Le Carnet du Chef")}">✉ Préparer un email</a>` : ""}</div>
      <div class="admin-alert" data-q-message hidden></div>
    </form>`;

  const form = editor.querySelector("[data-q-form]");
  const linesRoot = editor.querySelector("[data-q-lines]");
  const recalc = () => {
    const total = [...linesRoot.querySelectorAll("[data-line]")].reduce((sum, row) => sum + Number(row.querySelector("[name=quantite]").value || 0) * Number(row.querySelector("[name=prixUnitaire]").value || 0), 0);
    editor.querySelector("[data-q-total-editor]").textContent = money(total);
...
async function saveQuote(event, id, recalc) {
  ...
  try {
    if (id) await updateDoc(doc(db, "devis", id), payload); else { payload.createdAt = serverTimestamp(); const ref = await addDoc(collection(db, "devis"), payload); selectedId = ref.id; }
    await load();
  } catch (error) { const message = form.querySelector("[data-q-message]"); message.hidden = false; message.className = "admin-alert admin-alert-error"; message.textContent = `Enregistrement impossible : ${error.message}`; }
}

async function deleteQuote(id) {
  if (!id || !confirm("Supprimer définitivement ce devis ?")) return;
...
function init() {
  ...
  mount();
  auth.onAuthStateChanged((user) => { if (user) { mount(); load(); } });
}
